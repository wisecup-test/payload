...
- **R-UTIL-001** MUST: Public utility functions MUST be accompanied by unit tests covering normal cases, edge cases, and error conditions.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Automated CI pipeline checks for TypeScript strict mode compliance, code coverage gates, pull request reviews, and static analysis tools (ESLint) enforce these rules. Violations result in CI build failures, blocked pull requests, and automated comments identifying non-compliant utilities.
</enforcement>