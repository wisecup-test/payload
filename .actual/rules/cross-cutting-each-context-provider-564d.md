# Adopt React Context Providers for Component State Management in UI Boundaries: Each Context Provider
...
- **R-CONTEXT-001** MUST: Each Context Provider MUST be implemented in a dedicated Provider component file that encapsulates state management logic.
...
- Context files are named consistently (e.g., ComponentNameContext.tsx or Provider.tsx within component directory)
- Custom hooks are exported with descriptive names (e.g., useDrawerState, useTreeFocus) rather than generic useContext calls

<enforcement>
Claude Code MUST NOT skip or defer verification of Context Provider implementations. Code review checklist MUST include verification of Context usage for components with shared state. Violations MUST be marked as requiring changes before merge.
</enforcement>