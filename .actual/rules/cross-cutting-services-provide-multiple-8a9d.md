# Standardize Service Boundary Configuration Through Typed Config Objects: Services Provide Multiple

These rules are ALWAYS ACTIVE for all service modules, authentication configurations, queue workflows, UI components, relationship management services, API endpoint definitions, and test harness configurations that define service boundaries.

### Rules

- **R-SVC-001** MAY: Services MAY provide multiple configuration presets for different deployment environments or use cases.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. Configuration objects MUST be validated through type checking and test coverage before merge approval.
</enforcement>