# Adopt .spec.ts Extension for Test Files: Test Files Located
...
- **R-TEST-001** MUST: All TypeScript test files MUST use the .spec.ts extension.
- **R-TEST-002** SHOULD: Test files SHOULD be co-located with the code they test or organized in a dedicated test/ directory structure.
- **R-TEST-003** SHOULD: Integration tests SHOULD use the int.spec.ts naming pattern.
- **R-TEST-004** SHOULD: End-to-end tests SHOULD use the e2e.spec.ts naming pattern.
- **R-TEST-005** MUST: Test runner configurations MUST explicitly include **/*.spec.ts pattern in test discovery settings.
...
<enforcement>
Claude Code MUST NOT skip or defer verification. All test files must conform to the .spec.ts naming convention before code is accepted.
</enforcement>